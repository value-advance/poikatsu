...
**日本語の文章は、`<p>` `<li>` `<td>` などのタグ内で複数行に折り返して書かないこと。1文(1要素)は1行で記述する。**

```html
<!-- NG: ソースコード上の見やすさのために改行すると、ブラウザは改行+インデントを
     半角スペース1個として描画してしまう。日本語は単語間にスペースを入れないため、
     文中に意図しない半角スペースが表示されてしまう -->
<p>
  ボーナス月は普段より予算に余裕が出て、高額な買い物を
  検討しやすい時期です。
</p>

<!-- OK: 1文をまるごと1行で書く -->
<p>
  ボーナス月は普段より予算に余裕が出て、高額な買い物を検討しやすい時期です。
</p>
```

過去にこのルールに気づかず全記事で違反していたことがあり、6,800箇所以上の
不要な半角スペースが本文中に混入していた(2026.07にまとめて修正済み)。
新しく記事を作成する際は、既存記事をテンプレートとして参照する場合でも
この点だけは踏襲せず、必ず1行で記述すること。
